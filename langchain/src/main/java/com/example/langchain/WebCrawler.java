...
public class WebCrawler {
    ...
    private void ingestUrl(String url){
        ...
        final HtmlTextExtractor transformer = new HtmlTextExtractor(".td-container",
                Map.of("title", "h1.entry-title", "author", ".td-post-author-name",
                        "date", ".td-post-date", "visits", ".td-post-views"), true);
        final var transformedDocument = transformer.transform(document);

        DocumentSplitter splitter = DocumentSplitters.recursive(100, 5, new OpenAiTokenizer(OpenAiChatModelName.GPT_3_5_TURBO));
